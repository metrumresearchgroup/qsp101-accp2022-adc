...
TVVc   = 3.68e-12  // Volume of single cell
TVSc   = 1.66e-5   // Surface area of a single cell (cm^2)
Nc0  = 1.5e5     // Initial number of cells in well

// Rate constants

TVKon  = 0.0  // ADC/receptor on rate constant
// Koff = 1.0  // ADC/receptor off rate constant
TVKd   = 0.3   // ADC/receptor dissociation rate 
TVKdeg = 1.0  // Surface-bound receptor degradation rate
TVKint = 0.0  // ADC/receptor complex internalization rate constant
TVKend = 0.0  // Endosomal sorting rate for internalized ADC/receptor
TVKlys = 0.0  // Lysosomal degradation rate constant for internalized ADC/receptor
TVKrec = 0.0  // Rate for receptors recycling to surface
TVKrec_AR = 0.0 // Rate of AR complex recycling to surface
...
TVEC50_Payload = 1.0  // Killing due to payload
TVEmax_Payload = 0.0

EC50_ADCC    = 1.0  // Killing due to ADCC
Emax_ADCC    = 0.0
TVNr = 20e3 // Surface receptor expression (receptors/cell)


[OMEGA]
@labels E_EC50_Payload E_Emax_Payload E_Kd E_Kdeg E_Kend E_Kint E_Klys E_Kon E_Krec E_Krec_AR E_Nr E_Sc E_Vc
...
[MAIN]
double EC50_Payload = TVEC50_Payload * exp(E_EC50_Payload);
double Emax_Payload = TVEmax_Payload * exp(E_Emax_Payload);
double Kd = TVKd * exp(E_Kd);
double Kdeg = TVKdeg * exp(E_Kdeg);
double Kend = TVKend * exp(E_Kend);
double Kint = TVKint * exp(E_Kint);
double Klys = TVKlys * exp(E_Klys);
double Kon = TVKon * exp(E_Kon);
double Krec = TVKrec * exp(E_Krec);
double Krec_AR = TVKrec_AR * exp(E_Krec_AR);
double Nr = TVNr * exp(E_Nr);
double Sc = TVSc * exp(E_Sc);
double Vc = TVVc * exp(E_Vc);
...
double Ntot = Nc_1 + Nc_2 + Nc_3 + Nc_4;
...
// Flux = internalization - unbinding - transport to lysosome
...
dxdt_P_m = flux_P_c_to_P_m*Ntot + DAR*Kkill_eff*Nc_1*(A_e + A_l + AR_e + AR_s)/Nc_1;
...
[TABLE]
capture A_m_out = A_m * exp(EPS(1));
capture P_m_out = P_m * exp(EPS(2));
capture R_s_out = R_s * exp(EPS(3));
capture AR_s_out = AR_s * exp(EPS(4));
capture AR_e_out = AR_e * exp(EPS(5));
capture A_e_out = A_e * exp(EPS(6));
capture R_e_out = R_e * exp(EPS(7));
capture AR_l_out = AR_l * exp(EPS(8));
capture A_l_out = A_l * exp(EPS(9));
capture R_l_out = R_l * exp(EPS(10));
capture P_c_out = P_c * exp(EPS(11));
capture A_cat_out = A_cat * exp(EPS(12));
capture Nc_1_out = Nc_1 * exp(EPS(13));
capture Nc_2_out = Nc_2 * exp(EPS(14));
capture Nc_3_out = Nc_3 * exp(EPS(15));
capture Nc_4_out = Nc_4 * exp(EPS(16));
